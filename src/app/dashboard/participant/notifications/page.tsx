...
export default async function NotificationsPage() {
  ...
  const userDoc = await db.collection("users").findOne({ _id: new ObjectId(claims.userId) });
  const userProgramId = userDoc?.programId;
  ...
  if (userProgramId) {
    orConditions.push({ recipientId: new ObjectId(userProgramId) });
  }
  ...
  if (notifications.length > 0) {
    const unreadIds = notifications
      .filter((n: any) => !n.readBy?.some((id: any) => id.toString() === claims.userId))
      .map((n: any) => n._id);
